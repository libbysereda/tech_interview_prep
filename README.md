# Algorithms_and_Data_Structures
**Goal**: learn algorithms and data structures and prepare for a technical interview;

**Resources**:

*Theory*:
1. [Princeton Algorithms I](https://www.coursera.org/learn/algorithms-part1)
2. [Princeton Algorithms II](https://www.coursera.org/learn/algorithms-part2)
3. [Dr. Rob Edwards from San Diego State University](https://www.youtube.com/watch?v=zgCnMvvw6Oo&list=PLpPXw4zFa0uKKhaSz87IowJnOTzh9tiBk)
4. [Coding Interview University](https://github.com/jrenjenka/coding-interview-university)
5. [Booksite](https://algs4.cs.princeton.edu/home/)

*Practice*:
1. [HackerRank Data Structures](https://www.hackerrank.com/domains/data-structures)
2. [HackerRank Algorithms](https://www.hackerrank.com/domains/algorithms)
3. [HackerRank Mathematics](https://www.hackerrank.com/domains/mathematics)
2. [LeetCode](https://leetcode.com/problemset/algorithms/)
3. [InterviewBit Programming Track](https://www.interviewbit.com/courses/programming/)

**Daily plan**:

2 hours: solving problems;

1 hour: watching lectures;

Once a week review theory notes

## Modules Practice
Track Process:

**Done**

*In Progress*

**HackerRank Data Structures:**

- [ ] Arrays
- [ ] Linked Lists
- [ ] Trees
- [ ] Balanced Trees
- [ ] Stacks
- [ ] Queues
- [ ] Heap
- [ ] Disjoint Set
- [ ] Multiple Choice
- [ ] Trie
- [ ] Advanced

**HackerRank Algorithms:**
- [ ] Warmup
- [ ] Implementation
- [ ] Strings
- [ ] Sorting
- [ ] Search
- [ ] Graph Theory
- [ ] Greedy
- [ ] Dynamic Programming
- [ ] Constructive Algorithms
- [ ] Bit Manipulation
- [ ] Recursion
- [ ] Game Theory
- [ ] NP Complete
- [ ] Debugging

**HackerRank Mathematics:**

- [ ] Fundamentals
- [ ] Number Theory
- [ ] Combinatorics
- [ ] Algebra
- [ ] Geometry
- [ ] Probability
- [ ] Linear Algebra Foundations

**LeetCode:**

- [ ] Array
- [ ] Dynamic Programming
- [ ] Math
- [ ] String
- [ ] Tree
- [ ] Hash Table
- [ ] Depth-first Search
- [ ] Binary Search
- [ ] Two Pointers
- [ ] Breadth-first Search
- [ ] Greedy
- [ ] Stack
- [ ] Backtracking
- [ ] Design
- [ ] Linked List
- [ ] Bit Manipulation
- [ ] Heap
- [ ] Graph
- [ ] Sort
- [ ] Union Find
- [ ] Divide and Conquer
- [ ] Binary Search Tree
- [ ] Trie
- [ ] Recursion
- [ ] Queue
- [ ] Segment Tree
- [ ] Random
- [ ] Binary Indexed Tree
- [ ] Minimax
- [ ] Topological Sort
- [ ] Brainteaser
- [ ] Geometry
- [ ] Map
- [ ] Rejection Sampling
- [ ] Reservoir Sampling
- [ ] Memorization

**InterviewBit:**

- [ ] Time Complexity
- [ ] Arrays
- [ ] Two-dimensional arrays
- [ ] Dynamically allocated arrays
- [ ] Math
- [ ] Combinatorics
- [ ] Probability
- [ ] Divide-and-Conquer Algorithms
- [ ] Search
- [ ] Sorting
- [ ] Bit Manipulation
- [ ] Strings
- [ ] Two Pointers
- [ ] Linked Lists
- [ ] Stack and Queues
- [ ] Recursion
- [ ] Backtracking
- [ ] Hashing
- [ ] Heaps And Maps
- [ ] Disjoint Sets and Union-Find
- [ ] Tree Data Structure (AVL, Red-Black trees)
- [ ] Tries
- [ ] Dynamic Programming
- [ ] Greedy Algorithms
- [ ] Graph Data Structures & Algorithms
- [ ] NP-complete Problems
- [ ] Approximation Algorithms

## Modules Theory
**Princeton University and San Diego State University lectures:**

- [ ] Video: Dynamic Connectivity
- [ ] Video: Quick Find
- [ ] Video: Quick Union
- [ ] Video: Quick-Union Improvements
- [ ] Video: Union-Find Applications
- [ ] Practice Quiz: Interview Questions: Union-Find (ungraded)
- [ ] Union-Find
- [ ] Video: Analysis of Algorithms Introduction
- [ ] Video: Observations
- [ ] Video: Mathematical Models
- [ ] Video: Order-of-Growth Classifications
- [ ] Video: Theory of Algorithms
- [ ] Video: Memory
- [ ] Practice Quiz: Interview Questions: Analysis of Algorithms (ungraded)
- [ ] Video: Stacks
- [ ] Video: Resizing Arrays
- [ ] Video: Queues
- [ ] Video: Generics
- [ ] Video: Iterators
- [ ] Video: Stack and Queue Applications (optional)
- [ ] Practice Quiz: Interview Questions: Stacks and Queues (ungraded)
- [ ] Practice: Deques and Randomized Queues
- [ ] Video: Sorting Introduction
- [ ] Video: Selection Sort
- [ ] Video: Insertion Sort
- [ ] Video: Shellsort
- [ ] Video: Shuffling
- [ ] Video: Convex Hull
- [ ] Practice Quiz: Interview Questions: Elementary Sorts (ungraded)
- [ ] Video: Mergesort
- [ ] Video: Bottom-up Mergesort
- [ ] Video: Sorting Complexity
- [ ] Video: Comparators
- [ ] Video: Stability
- [ ] Practice Quiz: Interview Questions: Mergesort (ungraded)
- [ ] Practice: Graded: Collinear Points
- [ ] Video: Quicksort
- [ ] Video: Selection
- [ ] Video: Duplicate Keys
- [ ] Video: System Sorts
- [ ] Practice Quiz: Interview Questions: Quicksort (ungraded)
- [ ] Video: APIs and Elementary Implementations
- [ ] Video: Binary Heaps
- [ ] Video: Heapsort
- [ ] Video: Event-Driven Simulation (optional)
- [ ] Practice Quiz: Interview Questions: Priority Queues (ungraded)
- [ ] Practice: 8 Puzzle
- [ ] Video: Symbol Table API
- [ ] Video: Elementary Implementations
- [ ] Video: Ordered Operations
- [ ] Video: Binary Search Trees
- [ ] Video: Ordered Operations in BSTs
- [ ] Video: Deletion in BSTs
- [ ] Practice Quiz: Interview Questions: Elementary Symbol Tables (ungraded)
- [ ] Video: 2âˆ’3 Search Trees
- [ ] Video: Red-Black BSTs
- [ ] Video: B-Trees (optional)
- [ ] Practice Quiz: Interview Questions: Balanced Search Trees (ungraded)
- [ ] Video: 1d Range Search
- [ ] Video: Line Segment Intersection
- [ ] Video: Kd-Trees
- [ ] Video: Interval Search Trees
- [ ] Video: Rectangle Intersection
- [ ] Practice: Graded: Kd-Trees
- [ ] Video: Hash Tables
- [ ] Video: Separate Chaining
- [ ] Video: Linear Probing
- [ ] Video: Hash Table Context
- [ ] Practice Quiz: Interview Questions: Hash Tables (ungraded)
- [ ] Video: Symbol Table Applications: Sets (optional)
- [ ] Video: Symbol Table Applications: Dictionary Clients (optional)
- [ ] Video: Symbol Table Applications: Indexing Clients (optional)
- [ ] Video: Symbol Table Applications: Sparse Vectors (optional)
- [ ] Video: Introduction to Graphs
- [ ] Video: Graph API
- [ ] Video: Depth-First Search
- [ ] Video: Breadth-First Search
- [ ] Video: Connected Components
- [ ] Video: Graph Challenges
- [ ] Practice Quiz: Interview Questions: Undirected Graphs (ungraded)
- [ ] Video: Introduction to Digraphs
- [ ] Video: Digraph API
- [ ] Video: Digraph Search
- [ ] Video: Topological Sort
- [ ] Video: Strong Components
- [ ] Practice Quiz: Interview Questions: Directed Graphs (ungraded)
- [ ] Graded: WordNet
- [ ] Video: Introduction to MSTs
- [ ] Video: Greedy Algorithm
- [ ] Video: Edge-Weighted Graph API
- [ ] Video: Kruskal's Algorithm
- [ ] Video: Prim's Algorithm
- [ ] Video: MST Context
- [ ] Practice Quiz: Interview Questions: Minimum Spanning Trees (ungraded)
- [ ] Video: Shortest Paths APIs
- [ ] Video: Shortest Path Properties
- [ ] Video: Dijkstra's Algorithm
- [ ] Video: Edge-Weighted DAGs
- [ ] Video: Negative Weights
- [ ] Practice Quiz: Interview Questions: Shortest Paths (ungraded)
- [ ] Graded: Seam Carving
- [ ] Video: Introduction to Maxflow
- [ ] Video: Ford-Fulkerson Algorithm
- [ ] Video: Maxflow-Mincut Theorem
- [ ] Video: Running Time Analysis
- [ ] Video: Java Implementation
- [ ] Video: Maxflow Applications
- [ ] Practice Quiz: Interview Questions: Maximum Flow (ungraded)
- [ ] Practice: Graded: Baseball Elimination
- [ ] Video: Strings in Java
- [ ] Video: Key-Indexed Counting
- [ ] Video: LSD Radix Sort
- [ ] Video: MSD Radix Sort
- [ ] Video: 3-way Radix Quicksort
- [ ] Video: Suffix Arrays
- [ ] Practice Quiz: Interview Questions: Radix Sorts (ungraded)
- [ ] Video: R-way Tries
- [ ] Video: Ternary Search Tries
- [ ] Video: Character-Based Operations
- [ ] Practice Quiz: Interview Questions: Tries (ungraded)
- [ ] Video: Introduction to Substring Search
- [ ] Video: Brute-Force Substring Search
- [ ] Video: Knuth-Morris-Pratt
- [ ] Video: Boyer-Moore
- [ ] Video: Rabin-Karp
- [ ] Practice Quiz: Interview Questions: Substring Search (ungraded)
- [ ] Practice: Graded: Boggle
- [ ] Video: Regular Expressions
- [ ] Video: REs and NFAs
- [ ] Video: NFA Simulation
- [ ] Video: NFA Construction
- [ ] Video: Regular Expression Applications
- [ ] Practice Quiz: Interview Questions: Regular Expressions (ungraded)
- [ ] Video: Introduction to Data Compression
- [ ] Video: Run-Length Coding
- [ ] Video: Huffman Compression
- [ ] Video: LZW Compression
- [ ] Practice Quiz: Interview Questions: Data Compression (ungraded)
- [ ] Practice: Graded: Burrows-Wheeler
- [ ] Video: Introduction to Reductions
- [ ] Video: Designing Algorithms
- [ ] Video: Establishing Lower Bounds
- [ ] Video: Classifying Problems
- [ ] Practice Quiz: Interview Questions: Reductions (ungraded)
- [ ] Video: Brewer's Problem
- [ ] Video: Simplex Algorithm
- [ ] Video: Simplex Implementations
- [ ] Video: Linear Programming Reductions
- [ ] Practice Quiz: Interview Questions: Linear Programming (ungraded)
- [ ] Video: Introduction to Intractability
- [ ] Video: Search Problems
- [ ] Video: P vs. NP
- [ ] Video: Classifying Problems
- [ ] Video: NP-Completeness
- [ ] Video: Coping with Intractability
- [ ] Practice Quiz: Interview Questions: Intractability (ungraded)

## Worklog and Statistics

### Practice

***Solved:***

February:

25: [#727 - arrays](https://leetcode.com/explore/interview/card/top-interview-questions-easy/92/array/727/),

***Done with help***:

***Done by myself but need attention:***

***Need to be done:***
February:
25: [121. Best Time to Buy and Sell Stock - Kadane's Algorithm](https://leetcode.com/problems/best-time-to-buy-and-sell-stock/)

***To do next:***

### Theory

***Understood:***

February

- [x] Union-Find API and overview *11.02.2019, Mon*
- [ ] Quick Union;

***Need more information:***
